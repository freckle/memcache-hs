...
dzHeader pkt = do
  ...
  case pkt of
    PktResponse ->
      when (m /= 0x81)
        $ throw
        $ ProtocolError UnknownPkt {protocolError = show m}
    PktRequest ->
      when (m /= 0x80)
        $ throw
        $ ProtocolError UnknownPkt {protocolError = show m}
...
dzGetKResponse
  :: Header -> (Key -> Value -> Flags -> OpResponse) -> Get Response
...
dzNumericResponse h o = do
  v <-
    if status h == NoError && bodyLen h == 8
      then getWord64be
