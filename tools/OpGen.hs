...
data Operation = GET | STAT | NOOP deriving stock (Show, Eq)
...
options =
  [ Option
      ['q']
      ["qps"]
      (ReqArg (\q o -> o {qps = read q}) "QPS")
      "operations per quotum"
  , Option
      ['Q']
      ["quotum"]
      (ReqArg (\q o -> o {quotum = read q}) "SECONDS")
  ...
  , Option
      ['p']
      ["port"]
      (ReqArg (\p o -> o {port = read p}) "PORT")
      "port to connect to server on"
  , Option
      ['t']
      ["time"]
      (ReqArg (\t o -> o {time = read t}) "TIME")
...
main = do
  ...
  let
    step = (quotum opts * 1000000) `quot` qps opts
